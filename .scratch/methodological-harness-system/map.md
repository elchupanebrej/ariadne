...
- [Research the minimum graph-native harness contract](issues/14-research-graph-native-harness-contract.md) — Graph-native frame continuation is the ranked target: six readiness classes derivable from existing graph semantics with pointer-only handoff; teaching export trim is safe pre-first-publish; defer the persisted spine and dedicated runtime; no DEC lock until the Rung 3 table-driven check passes.
- [Record the graph-native continuation contract lock](specs/graph-native-continuation-and-package-repair.md) — The Rung 3 table-driven check passed (tickets 15–16), so the contract is locked as `DEC-graph-native-frame-continuation`: deepened status/report continuation with pointer-only handoff; the persisted spine and dedicated runtime are dispositioned as deferred, not falsified.
