...
`generateGrillSubstrate` card links now target the real persisted `GRAPH.jsonl` (never the substrate itself or invented per-card paths); `generateHandoff` Verified facts / Active invariants rows also carry inline links now. Recommended answers were already emitted per frontier question. Covered by `tests/e2e/uncertainty-flow.test.ts` (link existence + no-invented-paths assertions).
