...
const workspace = async () => {
  ...
  await graph.batch((batch) => {
    batch.appendEvents([
      ...
      { kind: "node", node: node("DEC-3", "DEC", "DECIDED") },
      {
        kind: "node",
      ...
      {
        kind: "node",
        node: node("DEC-SCOPE-A", "DEC", "DECIDED", { decision_scope: "auth-strategy" }),
      },
      {
        kind: "node",
        node: node("DEC-SCOPE-A2", "DEC", "DECIDED", { decision_scope: "auth-strategy" }),
      },
      {
        kind: "node",
        node: node("DEC-SCOPE-B", "DEC", "DECIDED", { decision_scope: "storage-engine" }),
      },
      { kind: "node", node: node("DEC-NO-SCOPE", "DEC", "DECIDED") },
      { kind: "node", node: node("CLM-1", "CLM", "DERIVED", { dependencies: ["DEC-1"] }) },
      { kind: "node", node: node("CAN-1", "CAN", "PROPOSED") },
      {
        kind: "node",
        node: node("ASM-TERM", "ASM", "ASSUMED", {
          status: "INVALIDATED",
          dependencies: ["DEC-1"],
        }),
      },
      ...
      { kind: "edge", edge: { source: "CAN-1", target: "DEC-1", type: "depends_on" } },
    ]);
  });
  return { cwd, graph };
};
...
describe("ariadne supersede", () => {
  it("supersedes a decision by another decision, updating graph, state, card, index, and reverse neighbors", async () => {
    ...
    const oldDec = materialized.nodes.find((n) => n.id === "DEC-1");
    ...
    const edge = materialized.edges.find(
  ...
  describe("decision scope matching", () => {
    ...
    it("allows supersession when both decisions declare matching decision_scopes", async () => {
      ...
      const materialized = await graph.materialize();
      expect(materialized.nodes.find((n) => n.id === "DEC-SCOPE-A")?.status).toBe("SUPERSEDED");
    });
    ...
    it("allows supersession when --by has decision_scope but target omits it", async () => {
      ...
      const materialized = await graph.materialize();
      expect(materialized.nodes.find((n) => n.id === "DEC-NO-SCOPE")?.status).toBe("SUPERSEDED");
    });
    ...
    it("allows supersession when neither decision declares a decision_scope", async () => {
      ...
      const materialized = await graph.materialize();
      expect(materialized.nodes.find((n) => n.id === "DEC-1")?.status).toBe("SUPERSEDED");
    });
  });
  ...
  it("serializes concurrent supersede operations under storage lock", async () => {
    ...
    const materialized = await graph.materialize();
    expect(materialized.nodes.find((n) => n.id === "DEC-1")?.status).toBe("SUPERSEDED");
    expect(materialized.nodes.find((n) => n.id === "DEC-2")?.status).toBe("SUPERSEDED");
  });
